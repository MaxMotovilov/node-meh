...
class MessageTarget {
	...
	pipe( message, handler, capped ) {
		this._msg[message] = { handler: handler, capped: capped }
		return this
	}

	sink( message, handler ) {
		return this.pipe( message, handler, true )
	}
...
class Request extends Stage {
	...
	get _upstream() {
		return (typeof this._up_req === "undefined" ? (this._up_req = new Request) : this._up_req)
	...
	_ready( capped_down ) {
		...
		if( capped_down ) {
			const up = this._up_req;		
			this._up_req = null;
			up && this._upstream; // Force failure
		}

		this._mt_down._connect( !capped_down && this._upstream._mt_down );
	...
	pipe( message, ...args ) {
		...
		return wrapper ? this[wrapper].apply( this, args ) : this._mt_down.pipe.apply( this._mt_down, arguments )
	}

	get sink() { return this._mt_down.sink }

	_wrapResponse( handler, capped ) {
	...
	_wrapUpstreamError( handler, capped ) {
		return (this._mt_up || (this._mt_up = new MessageTarget( this )))
	...
	body( body ) {
		const up = this._upstream;
		up && up._mt_down._dispatch( "body", body );
		return this
	}

	data( data ) {
		const up = this._upstream;
		up && up._mt_down._dispatch( "data", data );
		return this
	}
